/* app.js — Nikola Markovic Portfolio
   Vanilla JS: theme, audio, nav, reveal, mobile menu, smooth scroll, hero canvas
*/
...
(function () {
  ...
  const playBtn  = qs('#musicPlay');
  const pauseBtn = qs('#musicPause');
  ...
  playBtn?.addEventListener('click',  () => setAudioState(true));
  pauseBtn?.addEventListener('click', () => setAudioState(false));
  ...
  qsa('a[href^="#"]').forEach((anchor) => {
    anchor.addEventListener('click', (e) => {
      ...
      if (!target) return;
      e.preventDefault();
      const offset = parseInt(getComputedStyle(htmlEl).getPropertyValue('--nav-h'), 10) || 68;
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < CONNECT_D) {
            ...
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.strokeStyle = `rgba(${c[0]},${c[1]},${c[2]},${(1 - d / CONNECT_D) * 0.22})`;
            ctx.lineWidth = 0.8;
            ctx.stroke();
          }
        }
